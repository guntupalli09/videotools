 ...
 * Used by CLI script and POST /api/admin/recompute.
 */
...
/**
 * Recompute daily and monthly metrics for the given number of days and months.
 * @param days Number of days to recompute (back from today). Clamped 1–366.
 * @param months Number of months to recompute. Clamped 1–60.
 */
export async function runRecompute(days: number, months: number): Promise<RecomputeResult> {
  const safeDays = Math.max(1, Math.min(366, Math.floor(days) || 90))
  const safeMonths = Math.max(1, Math.min(60, Math.floor(months) || 12))

  const now = new Date()
  const todayStart = toUtcMidnight(now)
  const daysStart = addDays(todayStart, -safeDays)
  ...
  const monthStartFirst = firstDayOfMonth(addMonths(now, -safeMonths))
