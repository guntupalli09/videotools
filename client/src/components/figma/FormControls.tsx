  ...
  return (
    <label className="flex items-start gap-3 cursor-pointer group">
      ...
      <div className="flex-1">
        ...
        {description && <span className="text-xs text-gray-500 dark:text-gray-400 block mt-0.5">{description}</span>}
      </div>
    </label>
  );
}
...
export function ExportFormat({ formats, selected, onChange }: ExportFormatProps) {
  const toggleFormat = (format: string) => {
    if (selected.includes(format)) onChange(selected.filter((f) => f !== format));
    else onChange([...selected, format]);
  };
  return (
    <div className="space-y-3">
      <label className="text-sm font-semibold text-gray-900 dark:text-white block">Export Format</label>
